...
item is carried forward from `reports/FACULTY_EVALUATION_REPORT.md`'s own limitations
section (§15, itself consolidating STEP 11's reconciliation, which carries them forward
from STEP 1, 6, 7, and 10B). None is minimized, softened, or omitted here.
...
## Claims this evaluation deliberately does not make

Folded forward from `evaluation/archive/CONSOLIDATED_FACULTY_SUMMARY.md` (STEP 8) during
the PASS 2 cleanup — this list is more explicit and example-driven than the item-by-item
limitations above, and is kept for that reason:

- **"NyayaMind achieves 66.3% accuracy on natural data"** — 66.3% is evidence coverage,
  not accuracy, and no natural-data accuracy figure exists anywhere in this project.
- **"Evidence-v1 improved legal correctness"** — no legal ground truth exists to support
  this claim.
