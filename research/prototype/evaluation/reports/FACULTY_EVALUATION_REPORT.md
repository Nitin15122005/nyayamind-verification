...
12. Every step's raw outputs live under `research/prototype/evaluation/actual_outputs/`;
...
Per `ablation/ABLATION_EVIDENCE_GRADES.md` and `ablation/ABLATION_FACULTY_SUMMARY.md`
(moved from `evaluation/` in PASS 1),
...
`run_natural_209_paired_evaluation.py`'s own docstring (renamed from run_step6_209_paired_evaluation.py in PASS 3A): "No Qwen generation or correction
...
## Appendix: Technical reconciliation detail (STEP 11)

**PASS 2 note (2026-09-06)**: this appendix consolidates the two sections of the former
standalone `FINAL_RECONCILIATION_REPORT.md` that were not already covered elsewhere in
this document — that report's remaining content (machine-distinction narrative, GOLD/
natural-data/ablation/correction/safety restatements) duplicated §3-§11 above almost
verbatim and was not carried forward. The original file is preserved unedited at
`reports/archive/FINAL_RECONCILIATION_REPORT.md` for anyone who wants the STEP 11
document in its original, complete form.

### A.1 — STEP 1 → STEP 10B canonical source inventory

Every step's own canonical source artifact(s) and headline result, as reconciled at
STEP 11, with no new value invented at reconciliation time — every figure below was read
directly from its named source file:
...
| STEP 2 | `actual_outputs/step2_environment_setup/ENVIRONMENT_REPRODUCIBILITY_RESULT.md` | Venv built from `research/requirements.txt`, zero version substitutions; 205/205 pytest; the STEP 1-9 machine confirmed to have **no NVIDIA GPU** (AMD Radeon integrated only — see §3's separate STEP 10/10B machine) |
| STEP 3 | `inputs/README.md` (originally the separate file INPUT_MANIFEST.md, merged in PASS 1), `actual_outputs/step3_input_validation/validate_inputs_report.txt` | 38/38 input-integrity checks; GOLD-01 n=420, GOLD-02 n=59, hashes verified |
| STEP 4 | `actual_outputs/gold_benchmark_runs/{gold01_controlled,gold02_synthetic}/*_metrics.json` | GOLD-01/GOLD-02 bare vs. labeled accuracy/F1, bit-for-bit historical match |
| STEP 5 | `components/` (renamed from `component_tests/` in PASS 1), `evaluation/COMPONENT_TEST_MATRIX.csv` | 205/205 full suite; 7 component groups, 0 failures |
| STEP 6 | `actual_outputs/natural_data_runs/{588_claims,209_paired}/*_metrics.json` | 588-claim aggregate (66.3% coverage), 209-claim paired CPU re-verification (63.2%→70.3%, McNemar χ²=13.0667, p=0.000301) |
| STEP 7 | `ablation/ABLATION_SUMMARY.json`, `ABLATION_EVIDENCE_GRADES.md` (moved from `evaluation/` in PASS 1) | 9 levers graded A-E; joint four-lever isolation confirmed does not exist |
...
### A.2 — Claim safety audit (STEP 11)

A repository-wide search across every `evaluation/metrics/`, `evaluation/ablation/`, and
`evaluation/reports/` document for a fixed list of disallowed claim shapes — an accuracy or
F1 figure attributed to natural (non-GOLD) data, legal correctness asserted without a
GOLD label, a universal/absolute safety guarantee, the correction mechanism described as
a solved problem, GPU results said to establish correctness, a joint-lever interaction
effect, or a transfer of controlled-benchmark results onto natural data — found:
...
- **Zero GPU-absence statements lacking machine-scoping.** Every occurrence found (in
  `ENVIRONMENT_REPRODUCIBILITY_RESULT.md`, `EXPECTED_VS_ACTUAL_REPORT.md`, and — using
  the file paths as they existed at STEP 11, before the PASS 1/2 cleanup — the file then
  named `component_tests/03_verifier/RESULT.md` (now `components/03_nli_verification/RESULT.md`),
  the two STEP 8 summaries then living directly under `evaluation/` (now
  `evaluation/archive/CONSOLIDATED_EXECUTIVE_SUMMARY.md` and
  `evaluation/archive/CONSOLIDATED_FACULTY_SUMMARY.md`), the STEP 8 matrix then at
  `evaluation/REPRODUCIBILITY_MATRIX.md` (now `evaluation/archive/REPRODUCIBILITY_MATRIX.md`),
  and the file then named INPUT_TO_COMPONENT_MAP.md, under `inputs/` (now folded into
  `inputs/README.md`) — was explicitly scoped to "this machine," correctly describing the
  STEP 1-9 machine at the time it was written. None required correction; none was edited.
