...
final_demo_pack/ + final_comparison/
    ...
    |
    v
research/prototype/evaluation/  <-- this workspace
...
`inputs/README.md` (this section was originally `inputs/INPUT_MANIFEST.md`, merged into
`inputs/README.md` during the PASS 1 cleanup).
...
**Record counts and hashes**: see `inputs/README.md` (merged from `INPUT_MANIFEST.md` in
the PASS 1 cleanup) and `actual_outputs/step3_input_validation/validate_inputs_report.txt`
for the originally-recorded set (38 checks, all reported passed at the time): GOLD-01 = 420
(hash `aad8018bafc1d9e8b65b100b7cbbede6f22cfd29cf5c5d7863845443a632bfec`, byte-identical to
source), GOLD-02 = 59 (hash
`717378e88648deb8491caabc24455e1d7f57e27d8923ee2582a64936289237d5`, byte-identical to
source). **PASS 1.5 correction (2026-09-05)**: the two hashes above were originally
recorded as `962de21e...` and `2dba39cc...` respectively; a PASS 1.5 investigation
confirmed via full git history (`git log --follow`, `git show <commit>:<path> | sha256sum`)
that no version of either file was ever committed to this repository with those hashes —
the values above are the correct, sole, byte-identical hash of both the source and the
copy at every commit. See `PASS1_5_VERIFICATION_REPORT.md` for the full investigation.
v0 evidence = 63/63, v1 evidence = 82/82, merged usable pool = 136 (live-verified
...
`expected_outputs/` or `inputs/gold/` (the latter directory was merged into `inputs/README.md`
during the PASS 1 cleanup; this scan result describes what was true at STEP 3, when it
still existed as a separate directory).
...
**Validation command**:
```
research/.venv/Scripts/python.exe research/prototype/evaluation/scripts/validate_inputs.py
...
integration-test placement question (both flagged in STEP 1) remained open at this point
in the workspace's history — this step built and validated inputs only, no evaluation,
ablation, or comparison was run. **PASS 1 resolution (2026-09-05)**: the `evaluation/`
vs. `comparisons/metric_based/` question was closed — `comparisons/README.md` was
corrected to state that metric-based content lives directly in `evaluation/`, and no
`metric_based/` subdirectory was ever built or is now planned. The synthetic-set
integration-test placement question remains genuinely open — see `integration_tests/README.md`.
...
(`evaluation/scripts/run_gold01_evaluation.py`, `evaluation/scripts/run_gold02_evaluation.py`) call the real
verifier/pipeline functions directly; a shared helper (`evaluation/scripts/gold_benchmark_common.py`)
...
`scripts/run_controlled_benchmark.py`, outside `evaluation/`). See
`gold_benchmark_common.py`'s module docstring for the full technical detail and the resolution
...
`aad8018bafc1d9e8b65b100b7cbbede6f22cfd29cf5c5d7863845443a632bfec` (claimed at the time to
be "re-verified before inference, matching STEP 3's recorded hash exactly" — a PASS 1.5
investigation found this specific claim could not have been true against any
git-committed version of the file; see the PASS 1 correction note earlier in this
document and `PASS1_5_VERIFICATION_REPORT.md`). GOLD-02 =
`run_synthetic_stress.jsonl`, n=59, sha256
`717378e88648deb8491caabc24455e1d7f57e27d8923ee2582a64936289237d5` (same correction
applies). GOLD-02 additionally underwent a
...
**Actual output locations**: `evaluation/actual_outputs/gold_benchmark_runs/gold01_controlled/`,
`.../gold02_synthetic/`, `.../run_metadata/`, `.../logs/`.
**Expected-output locations**: unchanged from STEP 1/3 —
`evaluation/expected_outputs/controlled_benchmark_gold/`,
`evaluation/expected_outputs/synthetic_stress_gold/`.
**Comparison artifacts**: `evaluation/comparisons/expected_vs_actual/{gold01,gold02}_expected_vs_actual.csv`,
`gold_verifier_summary.csv`, `EXPECTED_VS_ACTUAL_REPORT.md`, `HISTORICAL_CROSSCHECK.md`.

**Validation**: `evaluation/scripts/validate_gold_benchmark_outputs.py` — **38/38 checks passed**, including
...
were added; see `components/TEST_INVENTORY.md` for the exact file/test-name mapping
(directory renamed from `component_tests/` in the PASS 1 cleanup, 2026-09-05 — see
`archive/cleanup_history/CLEANUP_PASS1_MANIFEST.md`).
...
`evaluation/scripts/run_component_demos.py`, re-runs these same real functions (never new
...
**Actual output locations** (as of this step): `evaluation/actual_outputs/step5_components/0N_*/`
(`pytest_execution.log`, `demo_examples.json`) and `full_suite_verbose.log`
(single source of truth for all pass/fail counts). **PASS 1 cleanup note (2026-09-05)**:
the `demo_examples.json` files were moved to `evaluation/components/0N_*/` (merged with the
STEP 3 methodology READMEs); the raw `pytest_execution.log`/`full_suite_verbose.log`
files were deleted as pure, reproducible console output once their counts were
reconciled against `components/COMPONENT_TEST_SUMMARY.md` — see
`archive/cleanup_history/CLEANUP_PASS1_MANIFEST.md`.
...
`run_metadata.json` (moved to `evaluation/components/` in the PASS 1 cleanup).
...
**Validation**: `evaluation/scripts/validate_step5_components.py (deleted in PASS 1)` (script deleted in the PASS 1
cleanup as an obsolete one-time validator once its outcome was preserved — see
`components/STEP5_VALIDATION_REPORT.txt` and `archive/cleanup_history/CLEANUP_PASS1_MANIFEST.md`) — **37/37 checks passed**,
...
`comparisons/metric_based/` ownership question (STEP 1) remained open at this point in
the workspace's history — this step performed component-level testing only. **Resolved
in PASS 1** (2026-09-05): see the STEP 3 entry above.

## STEP 6 — Natural-data metric-only evaluation (2026-09-05)

**Datasets used** (no substitution — exact datasets from `inputs/README.md`,
originally `inputs/INPUT_MANIFEST.md` before the PASS 1 merge):
...
originally stored). Full inventory: `evaluation/metrics/NATURAL_DATA_INVENTORY.md`.

**Input hashes / record counts**: verified before any inference via
`evaluation/scripts/verify_natural_input_integrity.py` — **zero discrepancies found** across 12
source files (4 with hashes carried over from STEP 3, matching exactly; 8 new baselines
recorded fresh this step). Full report: `evaluation/metrics/input_integrity/hash_report.json`.

**Exact commands**:
```
research/.venv/Scripts/python.exe research/prototype/evaluation/scripts/verify_natural_input_integrity.py
research/.venv/Scripts/python.exe research/prototype/evaluation/scripts/run_natural_588_evaluation.py
research/.venv/Scripts/python.exe research/prototype/evaluation/scripts/run_natural_209_paired_evaluation.py
research/.venv/Scripts/python.exe research/prototype/evaluation/scripts/analyze_natural_batches.py
...
`evaluation/metrics/GPU_LIMITATIONS.md`.
...
`evaluation/metrics/HISTORICAL_CROSSCHECK_NATURAL.md`.

**Paired analysis**: `evaluation/metrics/paired_209_analysis.csv`,
`evaluation/metrics/PAIRED_209_REPORT.md` — evidence gained (15), lost (0), unchanged (194);
...
**Validation**: `evaluation/scripts/validate_natural_data.py` — **33/33 checks passed**,
...
`evaluation/`, `final_comparison/`, `final_demo_pack/`, `tests/`, and `src/` for 8 factor
...
Full inventory: `ablation/ABLATION_INVENTORY.md` (moved from `evaluation/` in PASS 1).
...
3. Atomic scope check (assertion_spans) — the scope-check replay logic was **re-executed
   ...
   `evaluation/ablation/scope_check_replay_fresh.json` (moved from
   `actual_outputs/step7_ablation/` in PASS 1), and confirmed
...
**Exact commands**:
```
research/.venv/Scripts/python.exe research/prototype/evaluation/scripts/build_ablation_analysis.py
...
in `ablation/ABLATION_SUMMARY.json`):
...
`ablation/ABLATION_EVIDENCE_GRADES.md`, `ablation/ABLATION_RESULTS.md`.
...
requirement, in `ablation/ABLATION_RESULTS.md` and `ABLATION_FACULTY_SUMMARY.md`. No
...
**Validation**: `evaluation/scripts/validate_ablation.py` — **50/50 checks passed**,
...
any new number. Full map: `evaluation/metrics/CONSOLIDATION_SOURCE_MAP.md`.
...
**Figure-data files** (11, Phase 11, no PNGs generated): `evaluation/metrics/figure_data/01_overall_metric_comparison.csv`
...
```
research/.venv/Scripts/python.exe research/prototype/evaluation/scripts/build_metrics_consolidation.py
research/.venv/Scripts/python.exe research/prototype/evaluation/scripts/validate_metrics_consolidation.py   -> PASS, 75/75 checks
research/.venv/Scripts/python.exe research/prototype/evaluation/scripts/validate_figure_data.py           -> PASS, 54/54 checks
...
`evaluation/metrics/CONSOLIDATED_CONSISTENCY_REPORT.md` for the full reconciliation table. Two
...
STEP 1-7 — confirmed explicitly by `validate_metrics_consolidation.py`.
...
`evaluation/metrics/figure_data/*.csv` contracts.
...
**Figure-data sources**: all 11 `evaluation/metrics/figure_data/*.csv` files, re-verified via a
...
**Plotting script**: `evaluation/figures/generate_figures.py` — matplotlib only (no
...
through `11_synthetic_vs_natural_transfer.png`, all under `evaluation/figures/`.
...
**Validator results**: `evaluation/figures/audit_figures.py` — **PASS, 99/99 checks**
(automated: existence, opens, dimensions, resolution, blank/clipping heuristics,
metadata/source linkage). `evaluation/scripts/validate_figures.py` — **PASS, 66/66 checks**
...
Full data: `actual_outputs/gpu_capability_check/gpu_memory_probe_result.json`.
...
`actual_outputs/gpu_capability_check/run_modeA_n1.jsonl`.
...
`actual_outputs/gpu_capability_check/run_modeC_n5.jsonl`. Full per-case
detail: `evaluation/metrics/GPU_CORRECTION_SAFETY_STEP10.md`.
...
(`actual_outputs/gpu_correction_rerun/rerun_labeled_correction_validation_gpu.py`).
...
`evaluation/metrics/GPU_REPRODUCTION_CROSSCHECK.md`.
...
**Validation**: `evaluation/scripts/validate_gpu_experiments.py` — see this step's final
...
`evaluation/metrics/GPU_EXECUTION_INVENTORY.md`, `evaluation/metrics/GPU_REPRODUCTION_CROSSCHECK.md`,
`evaluation/metrics/GPU_CORRECTION_SAFETY_STEP10.md`, `ablation/GPU_ABLATION_UPDATE.md`,
`validate_gpu_experiments.py`, `actual_outputs/gpu_capability_check/*`,
`actual_outputs/gpu_correction_rerun/*`, and this PROVENANCE.md section.
...
reading `evaluation/scripts/run_natural_209_paired_evaluation.py`'s own docstring — that
...
**Fresh reproduction executed**: `evaluation/scripts/run_gpu_209_reproduction.py`
...
`evaluation/actual_outputs/gpu_209_reproduction/` (never touching
...
`evaluation/metrics/GPU_REPRODUCTION_CROSSCHECK.md`, Experiment 3.
...
**Validation**: `evaluation/scripts/validate_gpu_209_reproduction.py` — see this step's final
report for the exact PASS/FAIL count.

**Files added this step** (all new, nothing overwritten):
`evaluation/scripts/run_gpu_209_reproduction.py`,
`actual_outputs/gpu_209_reproduction/*` (fresh metrics/JSONL/console log),
`validate_gpu_209_reproduction.py`, and this PROVENANCE.md section. **Files extended
(append-only, no prior content removed)**:
`evaluation/metrics/GPU_REPRODUCTION_CROSSCHECK.md`, `ablation/GPU_ABLATION_UPDATE.md`.
No file under `research/data/`, `final_demo_pack/`, `final_comparison/`,
...
`components/0N_*/README.md` — renamed from `component_tests/` in the PASS 1 cleanup)
pointed at a file that existed in the commit this workspace was built against —
