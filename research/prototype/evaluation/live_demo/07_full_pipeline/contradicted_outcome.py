#!/usr/bin/env python
"""
Full-pipeline demo: a claim that reaches the CONTRADICTED final verdict.

Document: 1997_1306, source research/prototype/outputs/natural_candidates_batch2_gpu_labeled.jsonl.
Target citation: Section 148, Indian Penal Code, 1860 -- this document cites
Section 148 in TWO different sentences (claims c1 and c6); only c6 actually
contradicts its evidence (the model wrote "criminal trespass", the real
Section 148 IPC text is about rioting). Selection below is therefore by
BOTH provision number AND live verdict, not by provision number alone.
...
A CONTRADICTED verdict, in a real Mode "C" run, WOULD trigger a correction
attempt (src.pipeline._should_trigger_correction fires unconditionally for
CONTRADICTED) -- that full correction + re-verification + ship/reject
mechanism is demonstrated separately (and in more depth, using a different
real fixture) in ../06_correction_reverification_demo.py; it is not
repeated here.

Run:
  research/.venv/Scripts/python.exe research/prototype/evaluation/live_demo/07_full_pipeline/contradicted_outcome.py
"""
...
def main() -> int:
    fmt.section("FULL PIPELINE OUTCOME 2/4 -- CONTRADICTED")
    ...
    print(f"\n  Note: Section {TARGET_PROVISION_NUMBER} IPC is cited twice in this document "
          "(two different sentences) -- selecting the one that actually contradicts its "
          "evidence, not just the first Section-148 claim found.")
    ...
    print(f"\n  Would this verdict trigger a Mode-C correction attempt? {triggers} "
          "(CONTRADICTED always triggers correction, per "
          "src.pipeline._should_trigger_correction -- see "
          "../06_correction_reverification_demo.py for that full mechanism, "
          "including the SHIP-vs-REJECT safety gate.)")
    fmt.mode_b_outcome_line(
        target["verdict"],
        note="see ../06_correction_reverification_demo.py for what happens to a "
             "CONTRADICTED claim like this one in a real Mode-C run",
    )
