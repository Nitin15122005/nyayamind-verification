...
Run from the repo root:
    research/.venv/Scripts/python.exe research/prototype/evaluation/scripts/validate_metrics_consolidation.py
"""
...
ABLATION_DIR = TESTING_DIR / "ablation"  # PASS 1 cleanup: ABLATION_* files moved here from EVAL_DIR
...
def check_terminology():
    ...
    # PASS 2 cleanup: CONSOLIDATED_*.md, HEADLINE_RESULTS.md, ORIGINAL_VS_CURRENT.md, and
    # REPRODUCIBILITY_MATRIX.md moved to EVAL_DIR/archive/ (superseded by reports/
    # FACULTY_* documents) -- glob both locations so this check keeps covering them.
    files = list(EVAL_DIR.glob("CONSOLID*.md")) + list(EVAL_DIR.glob("HEADLINE*.md")) + \
            ...
            list((EVAL_DIR / "archive").glob("CONSOLID*.md")) + list((EVAL_DIR / "archive").glob("HEADLINE*.md")) + \
            list((EVAL_DIR / "archive").glob("ORIGINAL_VS_CURRENT.md")) + \
            list((EVAL_DIR / "archive").glob("REPRODUCIBILITY_MATRIX.md"))
...
def check_fresh_historical_gpu_separation():
    ...
    repro_path = EVAL_DIR / "archive" / "REPRODUCIBILITY_MATRIX.md"  # PASS 2: moved to archive/
