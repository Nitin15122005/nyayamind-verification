...
Writes only under research/prototype/evaluation/components/.

PASS 1 cleanup note (2026-09-05): this script originally wrote under
`actual_outputs/step5_components/<old-numbering>/`. That directory was consolidated
into `components/<canonical-8-stage-numbering>/` — see `archive/cleanup_history/CLEANUP_PASS1_MANIFEST.md`. The
`write_json` calls below target the new canonical paths directly; re-running this script
regenerates `demo_examples.json` (and, for the citation-adversarial case,
`citation_adversarial_demo_examples.json`) in place, alongside each stage's merged
README.md/RESULT.md.
"""
...
def demo_evidence_matcher():
    ...
    write_json("02_evidence_retrieval", "demo_examples.json", result)
    return result
...
def demo_citation_adversarial():
    ...
    write_json("02_evidence_retrieval", "citation_adversarial_demo_examples.json", result)
    return result
...
def demo_verifier_verdict_correction_and_assembly(real_verifier, exact_index, all_usable):
    ...
    write_json("03_nli_verification", "demo_examples.json", {"case": verifier_verdict_demo})
    ...
    write_json("06_scope_safety", "demo_examples.json", correction_safety_out)
    ...
    write_json("08_final_assembly", "demo_examples.json", final_assembly_out)
