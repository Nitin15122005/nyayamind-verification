...
ABLATION_DIR = TESTING_DIR / "ablation"  # PASS 1 cleanup: ABLATION_* outputs live here, not EVAL_DIR
OUT_DIR = ABLATION_DIR  # PASS 2 cleanup: scope_check_replay_fresh.json lives in ablation/, not actual_outputs/step7_ablation/ (deleted in PASS 1)
...
def factor_evidence_v1():
    m = load_json(TESTING_DIR / "actual_outputs" / "natural_data_runs" / "209_paired" / "paired_209_metrics.json")
...
def factor_premise_framing():
    step4_dir = TESTING_DIR / "actual_outputs" / "gold_benchmark_runs" / "gold01_controlled"
...
#    to evaluation/ instead of outputs/.
# ---------------------------------------------------------------------------
...
def factor_atomic_scope_check():
    ...
    findings.append({
        ...
        "fresh_or_historical": "FRESH REPRODUCTION (re-executed this run via the real, unmodified pipeline._scope_violation and claim_parser.extract_claims, output written to evaluation/ablation/scope_check_replay_fresh.json, cross-checked against the historical committed replay)",
...
def main():
    ...
    (ABLATION_DIR / "ABLATION_SUMMARY.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
    print(f"Wrote {ABLATION_DIR / 'ABLATION_SUMMARY.json'}")

    # ABLATION_MATRIX.csv (Phase 2) -- generated from the same findings
    matrix_path = ABLATION_DIR / "ABLATION_MATRIX.csv"
