...
Run from the repo root:
    research/.venv/Scripts/python.exe research/prototype/evaluation/scripts/validate_faculty_package.py
"""
...
def check_figures_exist():
    ...
    if missing:
        fail(f"{name}: missing {missing}")
        return
    ok(f"{name}: all 11 figure PNGs present under evaluation/figures/")
...
def _resolves(rel: str) -> bool:
    rel = rel.strip()
    candidates = [TESTING_DIR / rel, TESTING_DIR / rel.removeprefix("evaluation/"),
