...
its canonical source artifact by `validate_metrics_consolidation.py`'s
`check_all_26_headline_values` function — not eyeballed, not asserted from memory. Full
raw output: `evaluation/actual_outputs/step8_consolidation/validate_step8_report.txt`.
...
`../ablation/ABLATION_SUMMARY.json` (moved from `evaluation/` in PASS 1),
`archive/HEADLINE_RESULTS.md`, `archive/ORIGINAL_VS_CURRENT.md` (both moved to
`evaluation/archive/` in PASS 2), and all 11 `figure_data/*.csv` files). Per this step's
rule 15, had any discrepancy been found it would be reported here explicitly rather than
silently resolved — none was.

## Method

`validate_metrics_consolidation.py::check_all_26_headline_values` loads each of the 5
