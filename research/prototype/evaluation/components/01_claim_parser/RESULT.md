...
**Also see** `README.md` (STEP 3's stage-first mapping) and `../../inputs/README.md`'s
BEHAVIOR-inputs section (the classification-first view of the same tests).
...
**111 executed, 111 passed, 0 failed, 0 skipped.** (Raw pytest log retained only in
`PROVENANCE.md`'s STEP 5 narration and `COMPONENT_TEST_SUMMARY.md`'s reconciled counts —
the original per-component `pytest_execution.log` file was removed in the PASS 1 cleanup
as pure, reproducible console output; re-run the command above to regenerate it.)

## Representative input → actual output

Source: `demo_examples.json` in this directory (produced by re-running
`src/claim_parser.extract_citations`/`extract_claims` directly — not a new test, a
concrete demonstration using the exact sentence
