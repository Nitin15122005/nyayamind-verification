...
## PASS 1 cleanup note (2026-09-05)

This directory was consolidated from the original STEP 3/STEP 5 structure, which had
split the same 8 stages across two directories under different names
(`component_tests/0N_evidence_matcher` vs. `0N_evidence_retrieval`, etc. — two numbering
schemes for the same stage). Each stage now has exactly one directory here with, where
they exist, both its methodology `README.md` and its STEP 5 execution `RESULT.md` merged
together. No content was deleted in this consolidation — see
`../archive/cleanup_history/CLEANUP_PASS1_MANIFEST.md` for the full move/merge record. Per-component raw
`pytest_execution.log` files and workspace-level regression-recap logs were removed as
part of the same cleanup (pure, trivially-reproducible console output whose pass/fail
counts are already recorded in this file, `COMPONENT_TEST_SUMMARY.md`, and
`TEST_INVENTORY.md`); `run_metadata.json` and the STEP 5 validator report
(`STEP5_VALIDATION_REPORT.txt`) were kept and moved into this directory since they carry
unique reconciliation/provenance content.
...
`../PROVENANCE.md` for the full caveat list and `../reports/README.md` for where this
