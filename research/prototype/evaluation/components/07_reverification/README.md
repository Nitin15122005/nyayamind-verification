...
Same honest gap as `../05_correction/README.md` — STEP 5's 7-group execution taxonomy
folded re-verification into its "final assembly" group rather than giving it a separate
result. The closest existing empirical evidence is `../08_final_assembly/RESULT.md`'s
full end-to-end record, which shows a real re-verification call inside both the SHIP and
REJECT cases, and `test_correction_path_real_integration.py::test_full_correction_path_triggers_and_reverifies_with_real_verifier`.
...
`../../evaluation/CORRECTION_FUNNEL.md`.
