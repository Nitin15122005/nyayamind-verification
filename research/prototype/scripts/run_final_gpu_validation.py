#!/usr/bin/env python3
"""
Final pre-paper GPU validation: two-arm natural-data experiment on the
deterministic 50-case pool selected by
select_natural_candidates.py --out-suffix _final_validation
(outputs/natural_candidate_selected_ids_50_final_validation.json), which is
disjoint from every document_id used in any prior natural-data GPU
experiment (run_A/B/C_n30, run_natural_targeted, natural_candidates_50_gpu,
natural_candidates_batch2_gpu — 130 document_ids total, see
select_natural_candidates.py's PREVIOUSLY_EVALUATED_SOURCES).

Two arms, run over the SAME 50 cases with the SAME generation (Qwen is
called exactly once per case — generation depends on neither the evidence
pool nor any verification/correction config, so calling it twice would be
redundant, not more rigorous, exactly as run_natural_candidates_50_gpu.py
already established):

  Arm A ("baseline") — config/prototype.yaml's own on-disk defaults,
    completely unmodified: use_evidence_v1=false (59-record v0 pool),
    premise_framing=bare, correction.atomic_scope_check=false,
    correction.narrow_reverification_hypothesis=false,
    verification.confidence_threshold=0.70.

  Arm B ("improved") — every validated improvement from this project's prior
    diagnosis work enabled together: use_evidence_v1=true (138-record v0+v1
    pool), premise_framing=bare, correction.atomic_scope_check=
    "assertion_spans", correction.narrow_reverification_hypothesis=true,
    verification.confidence_threshold=0.70 (unchanged).

Both arms are built as deepcopy()s of the SAME config object loaded from
config/prototype.yaml; the file on disk is never written to, so production
defaults are untouched by this script by construction.

Because Arm B changes the EVIDENCE POOL itself (not just premise_framing,
which run_natural_candidates_50_gpu.py varied), evidence matching cannot be
shared between arms the way it was there — claim extraction (parsing) is
shared (it depends on generated text only), but evidence matching,
verification, and correction are run independently per arm against that
arm's own (exact_index, all_usable).
...
(read-only, never written). No prior output file is overwritten — every
output of this script uses the new "final_gpu_validation" prefix.
"""
...
def load_selected_cases(outputs: Path, repo_root: Path, config: dict, exact_index, all_usable,
    ...
    multi/single variant the selection script would have picked. Variant
    tie-break scoring always uses the v0-only pool (exact_index/all_usable
    passed in), matching exactly what select_natural_candidates.py itself
    used to build the ranking — this is a selection-time detail, independent
    of which arm's evidence pool is later used for matching/verification."""
...
def build_arm_claim_records(claims, exact_index, all_usable, fuzzy_threshold: float) -> list[dict]:
    """Re-implements the claim-record-building half of pipeline.generate_and_parse
    (evidence matching + record shape), applied to an ALREADY-extracted claims
    list, so the same parsed claims can be matched against two different
    evidence pools without calling the generator or the parser twice."""
    claim_records = []
    for claim in claims:
...
def classify_failure(status: str, reverification: dict | None) -> str | None:
    """Provenance-preserving classification of WHY a triggered correction did
    not ship, from the raw pipeline.apply_selective_correction() output alone
    (never re-derived from a different source):
      - "citation_identity_lost": the corrector's rewrite could not be matched
        back to the flagged claim's own citation at all (reverification is
        None only in this branch of pipeline.py) -> a citation-preservation
        failure.
      - "evidence_lost_after_edit": a replacement claim was found, but its
        (possibly altered) citation no longer resolves to any evidence.
...
def main() -> int:
    ...
    ap.add_argument("--out-prefix", default="final_gpu_validation")
    ...
    # Selection-time variant tie-break always uses the v0-only pool (arm A),
    # matching what select_natural_candidates.py itself used.
    cases = load_selected_cases(outputs, repo_root, base_config, exact_index["A"], all_usable["A"],
                                 args.candidate_file)
    ...
    result = {
        "experiment": "final_gpu_validation_A_vs_B",
    ...
    for arm in ARMS:
        with (outputs / f"{args.out_prefix}_{arm}.jsonl").open("w", encoding="utf-8") as f:
    ...
    print(f"Wrote {outputs / (args.out_prefix + '_A.jsonl')}")
    print(f"Wrote {outputs / (args.out_prefix + '_B.jsonl')}")
