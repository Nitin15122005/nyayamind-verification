...
filename itself matches the `-k` pattern). Full log:
`../../actual_outputs/step5_components/06_correction_safety/pytest_execution.log`.

## Chain demonstrated (real DeBERTa verifier + scripted corrector, CPU)

Source: `../../actual_outputs/step5_components/06_correction_safety/demo_examples.json`,
produced by re-running `src/pipeline.py::run_case` with the exact same fixtures
...
records (`06_correction_safety/demo_examples.json`). No citation-identity failure
