...
Run from the repo root:
    research/.venv/Scripts/python.exe research/prototype/testing/validate_step11_reconciliation.py
"""
...
RECON_REPORT = REPORTS_DIR / "FINAL_RECONCILIATION_REPORT.md"
...
def check_deliverables_exist():
    ...
    ok(f"{name}: FINAL_RECONCILIATION_REPORT.md, FINAL_CLAIM_REGISTER.csv, "
       f"FINAL_REPRODUCIBILITY_MATRIX.md all present under testing/reports/")
...
def _candidate_roots(rel: str) -> list[Path]:
    rel = rel.strip()
    return [TESTING_DIR / rel, TESTING_DIR / rel.removeprefix("testing/"),
