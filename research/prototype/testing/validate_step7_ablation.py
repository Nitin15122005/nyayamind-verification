...
def check_no_source_or_historical_changes():
    ...
    known_hashes = {
        "run_synthetic_stress.jsonl": "2dba39cc61eb94aa81daeff9817026ace0f531174e9312d49ce12522b3e9b516",
        "controlled_verifier_benchmark.jsonl": "962de21ee373bcfb685193902767c1c2c4f10acf592b08f561159503b4287f99",
    }
...
def check_no_gold_or_label_contamination():
    ...
    violation = False
    for d in [EVAL_DIR, TESTING_DIR / "actual_outputs" / "step7_ablation"]:
        for path in d.rglob("*"):
...
def check_stats_reconcile():
    ...
    summary_path = EVAL_DIR / "ABLATION_SUMMARY.json"
    ...
    if ev1:
        step6 = json.loads((TESTING_DIR / "actual_outputs" / "step6_natural_data" / "209_paired" / "paired_209_metrics.json").read_text(encoding="utf-8"))
...
def check_all_classified():
    ...
    summary = json.loads((EVAL_DIR / "ABLATION_SUMMARY.json").read_text(encoding="utf-8"))
...
def check_terminology():
    ...
    violation = False
    for md in list(EVAL_DIR.glob("ABLATION_*.md")):
...
def check_json_csv_md_agreement():
    ...
    summary = json.loads((EVAL_DIR / "ABLATION_SUMMARY.json").read_text(encoding="utf-8"))
    csv_text = (EVAL_DIR / "ABLATION_RESULTS.csv").read_text(encoding="utf-8")
    for f in summary["findings"]:
    ...
    md_text = (EVAL_DIR / "ABLATION_RESULTS.md").read_text(encoding="utf-8")
    for f in summary["findings"]:
