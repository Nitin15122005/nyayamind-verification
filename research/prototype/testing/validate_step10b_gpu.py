...
Run from the repo root:
    research/.venv/Scripts/python.exe research/prototype/testing/validate_step10b_gpu.py
"""
...
STEP10B_DIR = TESTING_DIR / "actual_outputs" / "step10b_gpu_experiments"
    ...
    for fname in ("step10b_final_gpu_validation_metrics.json",):
        if (PROTOTYPE_OUTPUTS / fname).exists():
            fail(f"{name}: fresh output {fname} was written into research/prototype/outputs/ "
                 f"instead of testing/actual_outputs/step10b_gpu_experiments/")
            return
    ok(f"{name}: final_gpu_validation_metrics.json and _A.jsonl SHA-256 hashes unchanged; "
       f"fresh outputs confined to testing/actual_outputs/step10b_gpu_experiments/")
