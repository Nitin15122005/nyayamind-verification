...
`research/prototype/testing/` workspace, STEP 1 through STEP 13, as it now stands. For
...
| `component_tests/` | Per-stage component test results | BEHAVIOR (205-test suite subset categorization) | `component_tests/` |
...
| `evaluation/HEADLINE_RESULTS.md` | STEP 8 headline narrative | Historical (STEP 8), consistent with STEP 11 reconciliation | `evaluation/HEADLINE_RESULTS.md` |
...
| `evaluation/ABLATION_EVIDENCE_GRADES.md` | Evidence-grade (A-E) assignment per ablation factor | HISTORICAL (STEP 7), unchanged by GPU work except where noted | `evaluation/ABLATION_EVIDENCE_GRADES.md` |
| `evaluation/ABLATION_FACULTY_SUMMARY.md` | Faculty-oriented ablation summary | HISTORICAL (STEP 7/8) | `evaluation/ABLATION_FACULTY_SUMMARY.md` |
| `evaluation/ORIGINAL_VS_CURRENT.md`/`.csv` (via `ABLATION_MATRIX.csv` etc.) | Original-vs-current config comparison | METRIC-ONLY/HISTORICAL | `evaluation/ORIGINAL_VS_CURRENT.md` |

## GPU validation and experiment reproduction (STEP 10/10B)

| Deliverable | Purpose | Status | Canonical source |
|---|---|---|---|
| `actual_outputs/step10_gpu_validation/` | Env validation, 1-case real generation, 5-case correction smoke test | FRESH, GPU-executed (STEP 10) | `actual_outputs/step10_gpu_validation/` |
| `actual_outputs/step10_gpu_experiments/` | 10-case targeted-correction exact reproduction | FRESH GPU REPRODUCED (STEP 10), exact match | `actual_outputs/step10_gpu_experiments/labeled_correction_validation_gpu_metrics.fresh.json` |
| `run_step10b_209_gpu_generation.py` | Redirected-output copy of the production 209-claim generation script | Tooling (STEP 10B), unmodified production logic | `run_step10b_209_gpu_generation.py` |
| `actual_outputs/step10b_gpu_experiments/` | Full 209-claim/50-case/two-arm fresh GPU generation | FRESH GPU REPRODUCED (STEP 10B), exact match (byte-identical text, identical metrics, identical peak VRAM) | `actual_outputs/step10b_gpu_experiments/step10b_final_gpu_validation_metrics.json` |
| `validate_step10_gpu.py`, `validate_step10b_gpu.py` | Automated checks for STEP 10/10B claims | PASS (13/13, 7/7 on last independent run) | same filenames |
...
| `reports/FINAL_REPRODUCIBILITY_MATRIX.md` | STEP 11 experiment-by-experiment reproduction grid | Current, supersedes STEP 8's `evaluation/REPRODUCIBILITY_MATRIX.md` (which is left unedited as its own historical record) | `reports/FINAL_REPRODUCIBILITY_MATRIX.md` |
| `validate_step11_reconciliation.py` | Automated checks for STEP 11 reconciliation | PASS (10/10 on last independent run) | `validate_step11_reconciliation.py` |
...
| `validate_step12_faculty_package.py` | Automated checks for the faculty package | PASS (9/9 on last independent run) | `validate_step12_faculty_package.py` |
...
| `validate_step13_release_audit.py` | Automated checks for the release audit | PASS (11/11) | `validate_step13_release_audit.py` |
