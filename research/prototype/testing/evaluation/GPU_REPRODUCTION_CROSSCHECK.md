...
- **Fresh source (STEP 10)**: `testing/actual_outputs/step10_gpu_experiments/rerun_labeled_correction_validation_gpu.py`
...
- **Fresh outputs**: `testing/actual_outputs/step10_gpu_experiments/labeled_correction_validation_gpu_metrics.fresh.json`,
...
- **Confirmed genuinely distinct from STEP 6**: `testing/run_step6_209_paired_evaluation.py`'s
...
- **Fresh source (STEP 10B)**: `testing/run_step10b_209_gpu_generation.py` —
  a copy of the historical script, logic byte-identical, with exactly two
  changes: (1) fresh outputs redirected to
  `testing/actual_outputs/step10b_gpu_experiments/` instead of
  ...
  `do_sample=false` (greedy/deterministic).
- **Fresh outputs**: `testing/actual_outputs/step10b_gpu_experiments/step10b_final_gpu_validation_{metrics.json,A.jsonl,B.jsonl,corrections_detail.jsonl}`,
...
- Its own `provenance` field states it was computed by "this session's
  ...
  `testing/build_step7_ablation_analysis.py` line ~379, which *loads* the
