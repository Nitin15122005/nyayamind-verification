...
## Scope, and how this differs from `component_tests/` and `evaluation/`

- `component_tests/` — one function/module in isolation, hand-constructed or mocked
  inputs, single-stage scope.
...
`research/prototype/final_demo_pack/live_demo/run_demo.py` is the other strong existing
asset: a real, GPU-free, code-path-verifying demo built for presentation purposes,
against already-generated text. It lives in a frozen directory and should be referenced
(and, when this workspace is ready, re-run and its output placed in
`../actual_outputs/` with the required run metadata) rather than copied.
