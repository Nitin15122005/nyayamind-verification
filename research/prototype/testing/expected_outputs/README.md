...
"expected behavior" used in `../component_tests/` (which means an exact code-level
...
stop — it belongs in `../comparisons/metric_based/` instead, described as a metric.
...
These live in `../component_tests/`, not here, and are a **different kind of thing**:
...
`../comparisons/metric_based/README.md` for how these are to be reported, and note in
particular:
