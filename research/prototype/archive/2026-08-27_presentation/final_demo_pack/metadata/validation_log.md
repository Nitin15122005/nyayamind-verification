...
Cause identified this pass (2026-09-09) as: the `examples/` and `live_demo/`
directories were "deleted from this pack in commit `61b240a`... while
`README.md`, `RUNBOOK.md`, and `ARTIFACT_INDEX.md` still describe them as
present." **This diagnosis was WRONG — see the 2026-09-11 addendum below,
which corrects it.** Not fixed this pass: reconstructing hand-written case
prose from (what was believed to be) deleted source data was judged out of
...
- All 2 JSON files under `final_demo_pack/` parse (down from "All 4" in the
  ...
  and `examples/cases.json`, both reported missing for the reason above)
...
because the script raised before reaching it).

## Addendum — 2026-09-11 correction: examples/ and live_demo/ were never deleted

The 2026-09-09 addendum above was **wrong**. Git archaeology (`git show -M
--name-status 61b240a`) shows `final_demo_pack/examples/*` and
`final_demo_pack/live_demo/*` were **renamed** (94-100% content similarity),
not deleted, to `research/prototype/evaluation/examples/` and
`research/prototype/evaluation/live_demo/` respectively — both fully intact
today, and `live_demo/` was actively extended afterward (commit `b83bb6b`
added `common/` helpers and 6 more per-stage demo scripts). The 2026-09-09
pass never checked the rename's destination, only that the old path was gone.

Real fix applied this pass: `validate_pack.py`'s
`spot_check_headline_numbers()` read `PACK_ROOT / "examples/cases.json"` —
always wrong for this archived copy, since `examples/` was never copied into
`archive/2026-08-27_presentation/` in the first place. Changed to read from
`REPO_ROOT / "research/prototype/evaluation/examples/cases.json"`, its real
location. `README.md`, `RUNBOOK.md`, and `ARTIFACT_INDEX.md` corrected to
point at the same location. Re-run after the fix:

**25/25 checks passed** (genuinely — via the path fix above, not by
weakening or removing the previously-failing check):

- All 2 JSON files under `final_demo_pack/` parse
- All 17 CSV files parse and are non-empty
- 16/16 figure PNGs present and non-trivial size
- 4/4 headline-number spot-checks against `computed_metrics.json` — all match
- **All 8 exemplar cases present in cases.json** — found 8 (previously
  unreached; now passes, reading from `research/prototype/evaluation/examples/cases.json`)
- **No undisclaimed 'lawyer-verified' claims found** — now passes (previously unreached)

See `final_demo_pack/README.md`'s own 2026-09-11 correction note for the full
explanation of the archaeology finding.

## Run — 2026-09-11T17:45:24.335432+00:00
Script: `research/prototype/archive/2026-08-27_presentation/final_demo_pack/metadata/validate_pack.py`

**25/25 checks passed.**

| Check | Result | Detail |
|---|---|---|
| All 2 JSON files under final_demo_pack/ parse | PASS |  |
| All 17 CSV files under final_demo_pack/ parse and are non-empty | PASS |  |
| 16 figure PNGs exist | PASS | found 16 |
| Figure 01_evidence_coverage_v0_to_v1.png is non-trivial size | PASS | 189199 bytes |
| Figure 02_evidence_coverage_across_natural_batches.png is non-trivial size | PASS | 166615 bytes |
...
## Run — 2026-09-11T17:54:05.300927+00:00
Script: `research/prototype/archive/2026-08-27_presentation/final_demo_pack/metadata/validate_pack.py`
...
## Run — 2026-09-11T21:15:34.550356+00:00
Script: `research/prototype/archive/2026-08-27_presentation/final_demo_pack/metadata/validate_pack.py`
...
## Run — 2026-09-11T21:55:51.703678+00:00
Script: `research/prototype/archive/2026-08-27_presentation/final_demo_pack/metadata/validate_pack.py`
...
## Run — 2026-09-12T06:42:33.795125+00:00
Script: `research/prototype/archive/2026-08-27_presentation/final_demo_pack/metadata/validate_pack.py`
...
## Run — 2026-09-12T07:34:30.874099+00:00
Script: `research/prototype/archive/2026-08-27_presentation/final_demo_pack/metadata/validate_pack.py`
...
## Run — 2026-09-12T08:25:02.842295+00:00
Script: `research/prototype/archive/2026-08-27_presentation/final_demo_pack/metadata/validate_pack.py`
...
## Run — 2026-09-12T08:31:53.559624+00:00
Script: `research/prototype/archive/2026-08-27_presentation/final_demo_pack/metadata/validate_pack.py`
